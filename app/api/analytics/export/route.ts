 ...
 *
 * This endpoint fetches analytics data and returns it as a downloadable CSV file.
 */

import { NextRequest, NextResponse } from 'next/server';
import { getServiceSupabase } from '@/lib/db/client';
import { exportVideoAnalyticsToCSV, generateExportFilename } from '@/lib/analytics/export-video-analytics';
 ...
 * - dateRange: { start: Date | string, end: Date | string } (required)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { creatorId, dateRange, format } = body;

    if (!creatorId || !dateRange) {
      return NextResponse.json(
        { error: 'Missing required parameters: creatorId, dateRange' },
        { status: 400 }
      );
    }
    ...
    // Fetch all data in parallel (same logic as videos/dashboard API)
    const [
      metricsResult,
      prevMetricsResult,
      viewsOverTimeResult,
    ...
    const dashboardData: VideoAnalyticsDashboardData = {
      metrics: {
        ...metricsResult,
        trends,
      },
    ...
    const filename = generateExportFilename({ start, end });
...
// Helper functions (same as videos/dashboard/route.ts)

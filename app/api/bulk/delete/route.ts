 ...
 * Bulk Delete API
 *
 * POST /api/bulk/delete - Initiate bulk video deletion
 */
 ...
 *
 * Initiate bulk deletion of videos
 */
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (video_ids.length > 100) {
      return NextResponse.json(
        { error: 'Cannot delete more than 100 videos at once' },
        { status: 400 }
      );
    }
    ...
    // Send Inngest event to process deletion
    await inngest.send({

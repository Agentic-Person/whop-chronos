 ...
 * Bulk Reprocess API
 *
 * POST /api/bulk/reprocess - Initiate bulk video reprocessing
 */
 ...
 *
 * Initiate bulk reprocessing of video embeddings
 */
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (video_ids.length > 100) {
      return NextResponse.json(
        { error: 'Cannot reprocess more than 100 videos at once' },
        { status: 400 }
      );
    }
    ...
    // Send Inngest event to process reprocessing
    await inngest.send({
      name: 'videos/bulk.reprocess',
      data: {
    ...
    console.error('Bulk reprocess API error:', error);
    return NextResponse.json(
      {

/**
 * Dashboard Entry Client Component
 *
 * This handles the client-side logic:
 * 1. Uses @whop/iframe SDK to get context (companyId, experienceId)
 * 2. Redirects to the appropriate dashboard
 *
 * The token has already been captured by the server component and stored in a cookie.
 */
...
interface DashboardEntryClientProps {
  hasToken: boolean;
}

export function DashboardEntryClient({ hasToken }: DashboardEntryClientProps) {
  const [status, setStatus] = useState<string>('Initializing...');
  const [debugInfo, setDebugInfo] = useState<Record<string, unknown>>({ hasToken });
  ...
  useEffect(() => {
    async function getContextAndRedirect() {
      try {
        ...
        // Token is already stored in cookie by the server component
        const data = urlData as { experienceId?: string; companyRoute?: string };
        const companyMatch = data.companyRoute?.match(/biz_[a-zA-Z0-9]+/);

        if (companyMatch) {
          window.location.href = `/dashboard/${companyMatch[0]}/overview`;
        } else if (data.experienceId) {
          window.location.href = `/experiences/${data.experienceId}/courses`;
        } else if (bizMatch) {
          setStatus('Using companyId from referrer...');
          window.location.href = `/dashboard/${bizMatch[0]}/overview`;
        } else if (expMatch) {
          setStatus('Using experienceId from referrer...');
          window.location.href = `/experiences/${expMatch[0]}/courses`;
        } else {

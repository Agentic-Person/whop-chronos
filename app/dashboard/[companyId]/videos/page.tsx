...
/**
 * Video Management Dashboard (Native Auth Version)
 *
 * Comprehensive video library management interface for creators
 * Uses Whop native authentication via the layout provider
 */
...
export default function VideosPage() {
  ...
  const fetchVideos = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/video/list?${searchParams.toString()}`);
  ...
  const handleVideoUpdate = async (id: string, updates: Partial<VideoType>) => {
    try {
...
function calculateProgress(status: VideoType['status']): number {
  const progressMap: Record<VideoType['status'], number> = {
    pending: 0,
    uploading: 10,
    transcribing: 30,
    processing: 50,

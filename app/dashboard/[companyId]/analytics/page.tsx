          ...
          <Card>
                ...
                <thead className="bg-gray-a2 border-b border-gray-a6">
                  <tr>
                    <th className="px-4 py-3 text-left">
                      <button onClick={() => handleSort('title')} className="flex items-center gap-2 text-3 font-medium text-gray-11 hover:text-gray-12">
                        Video <ArrowUpDown className="w-3 h-3" />
                      </button>
                    </th>
                    <th className="px-4 py-3 text-right">
                      <button onClick={() => handleSort('views')} className="flex items-center gap-2 ml-auto text-3 font-medium text-gray-11 hover:text-gray-12">
                        Views <ArrowUpDown className="w-3 h-3" />
                      </button>
                    </th>
                    <th className="px-4 py-3 text-right hidden sm:table-cell">
                      <button onClick={() => handleSort('completionRate')} className="flex items-center gap-2 ml-auto text-3 font-medium text-gray-11 hover:text-gray-12">
                ...
                <tbody className="divide-y divide-gray-a4">
                  {paginatedVideos.map((video) => (
                    <tr key={video.id} className="hover:bg-gray-a2 transition-colors">
                      ...
                      <td className="px-4 py-3 text-3 text-gray-12 text-right">{video.views.toLocaleString()}</td>
                      <td className="px-4 py-3 text-right hidden sm:table-cell">
                        <Badge color={video.completionRate >= 70 ? 'green' : video.completionRate >= 50 ? 'yellow' : 'red'}>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-gray-a6">
                ...
                <div className="flex items-center gap-2">
                  <Button variant="ghost" size="2" onClick={() => setCurrentPage(Math.max(1, currentPage - 1))} disabled={currentPage === 1}>
                    <ChevronLeft className="w-4 h-4" />
                  </Button>
                  <span className="text-3 text-gray-12">Page {currentPage} of {totalPages}</span>
                  <Button variant="ghost" size="2" onClick={() => setCurrentPage(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages}>
        ...
        <div className="space-y-6 mt-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className="p-4">
              ...
              <p className="text-6 font-bold text-gray-12">{data.chatAnalytics.totalSessions.toLocaleString()}</p>
            </Card>
            <Card className="p-4">
              <div className="flex items-center justify-between mb-2">
              ...
              <p className="text-6 font-bold text-gray-12">{data.chatAnalytics.totalMessages.toLocaleString()}</p>
            </Card>
            <Card className="p-4">
              <div className="flex items-center justify-between mb-2">
              ...
              <p className="text-6 font-bold text-gray-12">{data.chatAnalytics.avgMessagesPerSession}</p>
            </Card>
            <Card className="p-4">
              <div className="flex items-center justify-between mb-2">
              ...
              <p className="text-6 font-bold text-gray-12">${data.chatAnalytics.costPerSession.toFixed(2)}</p>
            </Card>
          </div>
          ...
          <Card className="p-6">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={data.engagementOverTime}>
                ...
                <XAxis dataKey="date" stroke="var(--gray-11)" style={{ fontSize: '12px' }} />
                <YAxis stroke="var(--gray-11)" style={{ fontSize: '12px' }} />
                <Tooltip contentStyle={{ backgroundColor: 'var(--gray-2)', border: '1px solid var(--gray-a6)', borderRadius: '8px' }} />
                <Legend />
                <Line type="monotone" dataKey="activeUsers" stroke="var(--blue-9)" strokeWidth={2} dot={{ r: 3 }} />
                <Line type="monotone" dataKey="sessions" stroke="var(--purple-9)" strokeWidth={2} dot={{ r: 3 }} />
              </LineChart>
            </ResponsiveContainer>
          </Card>
        </div>
...
function StatCard({ icon, label, value, trend, trendLabel }: { icon: React.ReactNode; label: string; value: string; trend: number; trendLabel: string }) {
